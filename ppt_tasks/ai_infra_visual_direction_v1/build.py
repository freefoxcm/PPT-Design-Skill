...
OUT = ROOT / "output" / "ai_infra_visual_direction_v1.pptx"
C = {"ink":"#071321","ink2":"#0D2237","paper":"#F4EFE6","muted":"#9DA8B4","rule":"#294258","cyan":"#2DE2E6","red":"#FF665C","lime":"#D9F45B","green":"#85B66A","white":"#FFFFFF"}

def t(s,x,y,w,h,v,size=14,color="paper",bold=False,font="Aptos",align=None):
...
def base(prs,num,section,title,subtitle=None):
    ...
    if title: t(s,0.82,0.92,11.3,0.58,title,24,"paper",True)
    if subtitle: t(s,0.84,1.53,10.8,0.25,subtitle,11,"muted")
    t(s,0.84,7.04,5.5,0.16,"AI INFRASTRUCTURE ECONOMICS / VISUAL DIRECTION V1",8,"muted",False,"Consolas")
...
def main():
    ...
    line(s,0.84,3.62,1.65,0.06,"red"); m(s,0.84,4.02,3.9,0.8,["A visual baseline for capacity,","compute, energy, and operating choice."],13,"paper")
    t(s,7.15,1.72,3.6,0.22,"THE VALUE CHAIN IS PHYSICAL",9,"muted",True,"Consolas")
    for y,w,label,accent in [(2.15,4.8,"APPLICATIONS","green"),(2.85,4.25,"PLATFORM","cyan"),(3.55,3.7,"COMPUTE","red"),(4.25,3.15,"POWER + PLACE","paper")]:
    ...
    t(s,7.38,5.1,4.45,0.5,"PHYSICAL\nBEFORE DIGITAL",27,"paper",True,"Consolas")
    ...
    for i,(x,label,body,accent) in enumerate(stages,1):
        t(s,x,2.78,0.45,0.2,f"0{i}",11,accent,True,"Consolas"); line(s,x,3.2,2.15,0.07,accent); t(s,x,3.55,2.2,0.26,label,18,"paper",True,"Consolas"); m(s,x,4.0,2.2,0.55,[body,"lead time ≠ demand"],11,"muted")
    ...
    rect(s,1.1,2.75,3.1,1.35,fill="#3A2023",C=C); rect(s,4.25,2.75,3.1,1.35,fill="#2A2030",C=C); rect(s,1.1,4.15,3.1,1.35,fill="#24372E",C=C); rect(s,4.25,4.15,3.1,1.35,fill="#21313F",C=C)
    ...
    for x,y,label,body,accent in [(1.35,3.0,"STRANDED","fixed cost · no flexibility","red"),(4.5,3.0,"BOTTLENECK","demand exists · supply lags","lime"),(1.35,4.4,"UNDERUSE","capacity exists · demand is soft","green"),(4.5,4.4,"HEALTHY","matched load · optionality","cyan")]:
        t(s,x,y,2.5,0.2,label,14,accent,True,"Consolas"); t(s,x,y+0.42,2.5,0.18,body,10,"paper")
    line(s,8.75,2.6,0.06,3.2,"red"); t(s,9.15,2.65,2.5,0.18,"WATCH ITEM",10,"red",True,"Consolas"); t(s,9.15,3.15,3.0,1.0,"Lead time\ncan outlive\ndemand.",24,"paper",True); m(s,9.15,4.55,2.6,0.65,["Hold capacity as an option.","Prove demand before ownership."],10,"muted")
